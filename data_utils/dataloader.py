...
class OCRDataset(Dataset):
    ...
    def __getitem__(self, index):
        '''
        samples: [{"image": image file, "label": ["B", "a", "o", " ", "g", "ồ", "m"]}, ... ]
        '''
        ...
        return img, tokens, torch.tensor(len(label))
...
class Batch:
    ...
    def __init__(self, imgs, tokens, token_lens, device="cuda"):
        self.imgs = imgs.to(device)
        self.tokens = tokens.to(device)
        self.tgt_len = token_lens.to(device)