...
def collate_fn(samples):
    ...
    for sample in samples:
        image, token, tokens_len = sample
        images.append(image)
        tokens.append(token)
        token_lens.append(tokens_len)
    ...
    token_lens_tensor = torch.cat([token_len.unsqueeze(0) for token_len in token_lens], dim=0)

    return images, tokens_tensor, token_lens_tensor
